...
public:
    ...
    virtual int RelayRx(u_char *data, size_t len) { (void)data; (void)len; if (m_rxChannel) { return m_rxChannel->OnHandleRx(data, len); } return NSPR_ERROR; }
...
{
public:
    int OnHandleTx(u_char *data, size_t len) { (void)data; (void)len; return NSPR_OK; }
