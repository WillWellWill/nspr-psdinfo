...
var PortMgmt = {
	...
	Update : function(pd) {
		if (!pd.windowGap) {
			...
			pd.dui = DiagramUI.createNew(pd.sd, 2000, 1000);
		}
		if (pd.completeShow) {
...
var StreamStatistics = {
    Update : function() {
		ApiProcNetwork.sendRequest("http://" + MgmtInitialize.currentIP() + ":7777/api/statistics/stream/query", StreamStatistics.OnUpdate);
	    var t = setTimeout("StreamStatistics.Update();", 1000);
    },
    ...
    OnUpdate : function(response, pd) {
        var obj = JSON.parse(response);
		$("#stream-statistics").empty();
        var myth = "<th class='statistics-record'>";
        var optionHtml = "";
        optionHtml += "<div class='statistics-container'>";
        optionHtml += "<table width='300' border='1' cellspacing='0' cellpadding='5'>";
        optionHtml += "<tr>";
        optionHtml += myth;
        optionHtml += "source addr"
        optionHtml += "</th>";
        optionHtml += myth;
        optionHtml += "dest addr";
        optionHtml += "</th>";
        optionHtml += myth;
        optionHtml += "total bytes";
        optionHtml += "</th>";
        optionHtml += "</tr>";
		for (i in obj) {
            if (obj[i].peer == "" && obj[i].local == "") {
                continue;
            }
            if (obj[i].received) {
                optionHtml += "<tr>";
                optionHtml += myth;
                optionHtml += obj[i].peer;
                optionHtml += "</th>";
                optionHtml += myth;
                optionHtml += obj[i].local;
                optionHtml += "</th>";
                optionHtml += myth;
                optionHtml += StreamStatistics.format(obj[i].received);
                optionHtml += "</th>";
                optionHtml += "</tr>";
            }
            if (obj[i].sent) {
                ...
                optionHtml += "</th>";
                optionHtml += myth;
                optionHtml += StreamStatistics.format(obj[i].sent);
                optionHtml += "</th>";
                optionHtml += "</tr>";
            }
		}
        optionHtml += "</table>";
        optionHtml += "</div>";
		$("#stream-statistics").append(optionHtml);
    }
};
